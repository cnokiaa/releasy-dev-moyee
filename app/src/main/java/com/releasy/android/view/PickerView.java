...
public class PickerView extends View
{
	...
	private int mColorText = 0x35C8FA;
	...
	private boolean isSlide = true; //是否可以滑
	...
	private void drawData(Canvas canvas)
	{
		...
		int numerical = Integer.parseInt(mDataList.get(mCurrentSelected));
		int drawLeftTime = numerical - 1;
		if(drawLeftTime > 2) drawLeftTime = 2;
		int drawRightTime = maxValue - numerical;
		if(drawRightTime > 2) drawRightTime = 2;
		
		for (int i = 1; i <= drawLeftTime; i++){	
			drawOtherText(canvas, i, -1, numerical-i);
		}
		// 绘制下方data
		for (int i = 1; i <= drawRightTime; i++){
			drawOtherText(canvas, i, 1, numerical+i);
		}
	 ...
	 *            1表示向下绘制，-1表示向上绘制
	 */
	private void drawOtherText(Canvas canvas, int position, int type, int value)
	{
		//scale都为0
		//Log.d("Z17M", "mMoveLen = " + mMoveLen);
		...
		float x = (float) (mViewWidth / 2.0 + position*type*mViewWidth/2.0/3.0 + mMoveLen);
	...
	private void doMove(MotionEvent event)
	{
		...
		if (mMoveLen > MARGIN_ALPHA * mMinTextSize / 2)
		{
			// 往下滑超过离开距离
			int numerical = Integer.parseInt(mDataList.get(mCurrentSelected));
			if(numerical > 1) moveTailToHead();;
			mMoveLen = mMoveLen - MARGIN_ALPHA * mMinTextSize;
		} else if (mMoveLen < -MARGIN_ALPHA * mMinTextSize / 2)
		{
			// 往上滑超过离开距离
			int numerical = Integer.parseInt(mDataList.get(mCurrentSelected));
			if(numerical < maxValue) moveHeadToTail();
			mMoveLen = mMoveLen + MARGIN_ALPHA * mMinTextSize;
		}
